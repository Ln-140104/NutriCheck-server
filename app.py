...
COIN_DIAMETER_CM = 2.7   # Indian 10-rupee coin

app = FastAPI(title="Food Estimator API", version="1.0.0")
app.add_middleware(
    CORSMiddleware,
...
def detect_scale(image_path):
    ...
    for result in results:
        ...
        for cls, mask in zip(result.boxes.cls, result.masks.data):
            if "coin" in result.names[int(cls)].lower():
    ...
    if not coin_candidates:
        raise ValueError("No coin detected in top-view image.")
...
def get_mask(image_path, food_key):
    food = FOOD_DB[food_key]
    results = model.predict(image_path, imgsz=640, conf=0.15)
    img = cv2.imread(image_path)
    img_h, img_w = img.shape[:2]
    ...
    for result in results:
        if result.masks is None:
            continue
        for cls, mask in zip(result.boxes.cls, result.masks.data):
            if food["class_name"] in result.names[int(cls)].lower():
                mask_np = mask.cpu().numpy().astype(np.uint8) * 255
                mask_np = cv2.resize(mask_np, (img_w, img_h), interpolation=cv2.INTER_NEAREST)
                mask_total = cv2.bitwise_or(mask_total, mask_np)
    if np.sum(mask_total) == 0:
        raise ValueError(f"No {food['name']} detected in image.")
    return mask_total
...
def measure_top(mask_total, food_key, scale):
    ...
    width_px  = float(proj_short.max() - proj_short.min())
    return length_px * scale, width_px * scale
...
def compute_nutrients(food_key, weight_g):
    n = FOOD_DB[food_key]["nutrition"]
    f = weight_g / 100.0
    return {k: round(v * f, 2) for k, v in n.items()}
...
async def estimate(
    ...
    if food not in FOOD_DB:
        raise HTTPException(400, f"Unknown food '{food}'. Choose from: {list(FOOD_DB.keys())}")
    ...
    try:
        ...
        top_mask   = get_mask(top_path,  food)
        side_mask  = get_mask(side_path, food)
