...
app = FastAPI(title="NutriCheck API", version="1.0.0")
app.add_middleware(CORSMiddleware, allow_origins=["*"], allow_methods=["*"], allow_headers=["*"])

def get_predictions(image_path, conf=0.15):
    return model.predict(image_path, imgsz=640, conf=conf, verbose=False)

def detect_scale(image_path):
    results = get_predictions(image_path, conf=0.3)
    ...
    if not coin_candidates:
        raise ValueError("No coin detected. Make sure the Rs.10 coin is clearly visible in the top view.")
...
def get_food_mask(image_path, food_key):
    ...
    results = get_predictions(image_path, conf=0.15)
    img = cv2.imread(image_path)
    img_h, img_w = img.shape[:2]
...
def measure_top(mask_total, food_key, scale):
    ...
    return float(proj_long.max() - proj_long.min()) * scale, float(proj_short.max() - proj_short.min()) * scale
...
def compute_nutrients(food_key, weight_g):
    return {k: round(v * weight_g / 100.0, 2) for k, v in FOOD_DB[food_key]["nutrition"].items()}

@app.get("/health")
def health(): return {"status": "ok", "classes": model.names}

@app.get("/foods")
def list_foods():
    return {k: {"name": v["name"], "nutrition_per_100g": v["nutrition"]} for k, v in FOOD_DB.items()}
